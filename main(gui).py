...
#주문완료, 주문취소 버튼 클릭시 함수 연결
        self.bt_order.clicked.connect(self.statistics_write)
        self.bt_order.clicked.connect(self.order_ok)
        self.bt_order_cancer.clicked.connect(self.order_cancer)

#메니저 패스 클릭시 함수연결(암호확인)
        self.ac_manager_pass.triggered.connect(self.manager_pass)

#통계 버튼 클릭시 함수 연결
        self.bt_statistics_show.clicked.connect(self.statistics_show)
        self.bt_statistics_reset.clicked.connect(self.statistics_reset)
        self.bt_statistics_cocktail.clicked.connect(self.statistics_cocktail)
...
#주문완료 버튼시 함수
    def statistics_write(self) :
        aaaa = self.le_when.text()
        order_time = datetime.datetime.now().strftime('%Y년 %m월 %d일 %H시 %M분')
...
#주문 함수
    def order_ok(self):
        ...
        QMessageBox.question(self, '주문확인', f'{self.tb_order.toPlainText()}\n주문이 맞습니까?', QMessageBox.Yes | QMessageBox.No, QMessageBox.No)

#주문취소 함수
    def order_cancer(self):
...
#메니저 암호 함수
    def manager_pass(self):
        m_pass, ok = QInputDialog.getText(self, '메니저모드', '암호를 입력하세요:')
        if ok:
            if m_pass == '007':
                self.gb0.setEnabled(True)
            else :
                QMessageBox.warning(self, '메니저모드', '암호가 틀렸습니다', QMessageBox.Yes)# | QMessageBox.No, QMessageBox.No)

#기록보이기 함수
    def statistics_show(self):
        statistics_date = self.le_when.text()
        self.tb_statistics.setText('')
        for i in 기록.keys():
            if statistics_date in i:
                aaa = str(i+'  -  '+기록[i])
                bbb = aaa.replace('\n', ', ')
                self.tb_statistics.append(bbb)
            else:
                self.tb_statistics.setText('기록이 없습니다')
